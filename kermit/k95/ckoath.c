...
static void CKCRYPTAPI callback_install_dllfunc(char * name, void * func)
{
...
static ttol_callback callback_ttol;
static dodebug_callback callback_dodebug;
static dohexdump_callback callback_dohexdump;
static tn_debug_callback callback_tn_debug;
static vscrnprintf_callback callback_vscrnprintf;
static RequestTelnetMutex_callback callback_RequestTelnetMutex;
static ReleaseTelnetMutex_callback callback_ReleaseTelnetMutex;

static int CKCRYPTAPI callback_ttol(char *s, int n) {
    return ttol(s, n);
}
static int CKCRYPTAPI callback_dodebug(int flag, char * s1, char * s2, CK_OFF_T n) {
    return dodebug(flag, s1, s2, n);
}
static int CKCRYPTAPI callback_dohexdump( char * s1, char * s2, int n ) {
    return dohexdump(s1, s2, n);
}
static void CKCRYPTAPI callback_tn_debug(char *s) {
    tn_debug(s);
}
static int CKCRYPTAPI callbackp_scrnprint(const char *str) {
    return scrnprint(str);
}
static unsigned long CKCRYPTAPI callback_RequestTelnetMutex(unsigned long x) {
    return RequestTelnetMutex(x);
}
static unsigned long CKCRYPTAPI callback_ReleaseTelnetMutex(void) {
    return ReleaseTelnetMutex();
}

#else /* CRYPT_DLL_CALLCONV */

#define callback_ttol 			ttol
#define callback_dodebug		dodebug
#define callback_dohexdump		dohexdump
#define callback_tn_debug		tn_debug
#define callback_scrnprint		scrnprint
#define callback_RequestTelnetMutex	RequestTelnetMutex
#define callback_ReleaseTelnetMutex	ReleaseTelnetMutex
...
ck_crypt_loaddll( void )
{
    ...
    if ( !hCRYPT ) {
        ...
        dllentryp_crypt_init = (crypt_dll_init_dllentry *)GetProcAddress( hCRYPT, "crypt_dll_init" );
    ...
    init.version = 6;
    /* Version 1 */
    init.callbackp_ttol = callback_ttol;
#ifndef NODEBUG
    init.callbackp_dodebug = callback_dodebug;
    init.callbackp_dohexdump = callback_dohexdump;
#else /* NODEBUG */
    init.callbackp_dodebug = NULL;
    init.callbackp_dohexdump = NULL;
#endif /* NODEBUG */
    init.callbackp_tn_debug = callback_tn_debug;
    init.callbackp_vscrnprintf = callback_vscrnprintf;
    /* Version 2 */
#ifdef KRB5
    ...
    /* Version 5 */
    init.callbackp_reqtelmutex = callback_RequestTelnetMutex;
    init.callbackp_reltelmutex = callback_ReleaseTelnetMutex;
    ...
    if ( dllentryp_crypt_init == NULL ||
         ...
         dllfuncp_libdes_fixup_key_parity==NULL ||
         dllfuncp_libdes_pcbc_encrypt==NULL
         /* dllfuncp_crypt_dll_version is an optional function */
         )
    {
        debug(F110,"K95 Crypt initialization failed",
